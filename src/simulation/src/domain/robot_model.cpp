...
std::expected<State, SimulationErrorCodes> RobotModel::step(
  const Velocity2D & velocity_cmd, double dt_s)
{
  ...
  const double long_beta = config_.long_velocity_time_constant_s /
    (config_.long_velocity_time_constant_s + dt_s);
  ...
  const double ang_beta = config_.angular_velocity_time_constant_s /
    (config_.angular_velocity_time_constant_s + dt_s);
  ...
  state.pose.theta_rad =
    previous_state_.pose.theta_rad + state.velocity.angular_radps * dt_s;
  state.pose.x_m =
    previous_state_.pose.x_m +
    state.velocity.linear_mps * std::cos(previous_state_.pose.theta_rad) * dt_s;
  state.pose.y_m =
    previous_state_.pose.y_m +
